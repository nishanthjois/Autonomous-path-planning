            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    if (too_close)
                    {
                        ...
                        {
                            lane = lane -1;
			    std::cout<< "Switched to left lane" << endl;
                        }
                        ...
                        {
                            lane = lane +1;
			    std::cout<< "Switched to right lane" << endl;
                        }
                        else
                        {
                            // Slow down - if not safe to switch lanes, so that we wont bump into car in front of our lane
                            target_speed -= acc; 
			    std::cout<< "Slow down" << endl;
                    ...
                    {
                        // Speed up - if there is no car in our lane 
                        target_speed += acc; 
			std::cout<< "Keep going ..." << endl;
                    }
          
                    // Widely spaced waypoints - spaced at 30m - we can interoplate these waypoints with a spine and fill with more points
                    // this controls car's speed.

                    vector<double> ptsx;
                    vector<double> ptsy;

                     // Refer the starting point as where the car is or at previous path's end point   
                    double ref_x = car_x; 
                    double ref_y = car_y;
                    double ref_yaw = deg2rad(car_yaw);

                    // if previous size is empty, use the car as starting ref
                    if (prev_size < 2)
                    {
                        //calculate the previous position of the car (which is tanget to angle of the car)
                    ...
                    // Calcuate where the car was heading using last couple of points of previous points
...
/*
                    ...
                    // Now the car has 7 points prev_ref, ref_, next_wp0,next_p1,next_wp1 in ptsx and ptsy [which is location of the car]
                    //ptsx.push_back(next_wp0[0]);
                    ptsx.push_back(next_wp1[0]);
                    ptsx.push_back(next_wp2[0]);
...
*/
                    ...
                    // Transform to local car co-oridnates [ which makes last co-ordinate is at 0,0 and angle is zero degree]    
                    ...
                    // Create a spline
                    // Here we will build the future path from our starting x,y
                    tk::spline s;
                    ...
                    vector<double> next_y_vals;
                     ...
                     //  Starter code:  instead of setting the speed directly, we pass next_x_vals, and next_y_vals to the simulator. 
